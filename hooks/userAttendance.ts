...
export function useAttendance(userId: string | null) {
  ...
  const clockOut = async (selfieUrl?: string, notes?: string) => {
    ...
    // Calculate work hours before clocking out
    const clockInTime = attendanceState.currentAttendance.clockIn.getTime();
    const now = new Date().getTime();
    const workHours = Math.floor((now - clockInTime) / (1000 * 60 * 60)); // Convert to hours and round down to integer
    ...
    const { error } = await attendanceService.clockOut({
      ...
      notes,
      workHours: workHours,
      breakTime: Math.floor(attendanceState.currentAttendance.breakTime) || 0,
      overtimeHours: Math.floor(attendanceState.currentAttendance.overtimeHours) || 0,
      clientVisitTime: Math.floor(attendanceState.currentAttendance.clientVisitTime) || 0
    });
    ...
    // Update local state to reflect clocked out status
    setAttendanceState(prev => ({
      ...prev,
  ...
  const addActivity = async (
    ...
    if (!error) {
      // Reload activities and current attendance
      await Promise.all([
        loadTodayActivities(),
